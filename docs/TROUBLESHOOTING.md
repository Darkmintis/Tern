...
| Play versionCode already used | Duplicate build number | Tern should prompt before build; or add `bump version build` to the Ternfile lane / `tern release --yes`, rebuild, upload |
...
| versionCode clash | Answer yes to the pre-build bump prompt, add `bump version build` in Ternfile, or `tern release --yes`, then rebuild |
