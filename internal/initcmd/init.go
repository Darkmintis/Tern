...
// Android-only workflow (Windows/Linux developers)
const ghaWorkflowAndroidOnly = `# Generated by tern init — Android release with Tern
...
// iOS-only workflow (macOS developers)
const ghaWorkflowIOSOnly = `# Generated by tern init — iOS release with Tern
name: Release iOS
on:
  workflow_dispatch:
jobs:
...
// Parallel workflow (macOS with both platforms)
const ghaWorkflowParallel = `# Generated by tern init — Parallel Android + iOS release with Tern
name: Release
on:
  workflow_dispatch:
jobs:
...
func Run(projectRoot string, reg *adapter.Registry, writeWorkflow bool) (Result, error) {
	...
	if writeWorkflow {
		...
		if _, err := os.Stat(wfPath); err != nil {
			// Generate workflow based on detected platforms
			workflow := ghaWorkflowAndroidOnly
			if d.HasAndroid && d.HasIOS {
				workflow = ghaWorkflowParallel
			} else if d.HasIOS && !d.HasAndroid {
				workflow = ghaWorkflowIOSOnly
			}
